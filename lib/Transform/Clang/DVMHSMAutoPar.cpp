...
void addVarList(const ReductionVarListT VarInfoList,
...
protected:
  explicit Pragma(const SourceLocation BL,
    const ASTRegionTraitInfo VarInfo, PragmaType PragmaType) :
...
public:
  explicit PragmaParallel(const SourceLocation BL,
...
public:
  explicit PragmaRegion(const SourceLocation BL, const SourceLocation EL,
    const ASTRegionTraitInfo VarInfo,
    std::vector<std::pair<const ForStmt*, Loop*>> Loops, bool IsHost) :
  ...
  Loop* getIRLoop(size_t I) const {
    return mLoops[I].second;
  }

  std::vector<std::pair<const ForStmt*, Loop*>> getLoops() const {
    return mLoops;
  }
...
private:
  ...
  std::vector<std::pair<const ForStmt*, Loop*>> mLoops;
};
...
bool findMemoryOverlap(T& P1, T& P2, Function& F,
  ...
  for (auto& CBB = F.begin(), EBB = F.end(); CBB != EBB; CBB++) {
    ...
    for (auto& I : CBB->instructionsWithoutDebug()) {
      ...
      for_each_memory(I, *TLI,
        [&Overlap, AT, DIMInfo, DT, &P1, &P2, TLI](Instruction& I, MemoryLocation&& Loc,
          unsigned Idx, AccessInfo, AccessInfo W) {
            if (W == AccessInfo::No)
              return;
            auto EM = AT->find(Loc);
...
public:
  ...
  void tryUnionRegions(const Function *F, tsar::TransformationContext& TfmCtx) {
    ...
    for (auto& item : mPragmas[F]) {
      ...
      std::copy_if(item.second.begin(), item.second.end(), std::back_inserter(Regions),
        [](const Pragma* Pragma) {
          return Pragma->getPragmaType() == PragmaType::Region; 
        });
      auto& I = item.second.begin(), N = item.second.begin();
      for (N++; N != item.second.end(); I++, N++) {
      ...
      for (auto& NewRegion : NewRegions) {
        ...
        std::vector<std::pair<const ForStmt*, Loop*>> Loops;
      ...
      // insert old pragmas
      std::copy_if(item.second.begin(), item.second.end(), std::back_inserter(NewPragmas),
        [](const Pragma* Pragma) {
          return Pragma->getPragmaType() == PragmaType::Parallel;
        });
      std::copy(SavedPragmas.begin(), SavedPragmas.end(), std::back_inserter(NewPragmas));

      // changes vectors of pragmas
      for (Pragma* Pragma : item.second) {
        if (Pragma->getPragmaType() != PragmaType::Parallel && 
          std::find(SavedPragmas.begin(), SavedPragmas.end(), Pragma) == SavedPragmas.end()) {
          delete Pragma;
        }
      }
  ...
  void tryUnionPragmas(Function* F, DIMemoryClientServerInfo& DIMInfo,
    ...
    for (auto& item : mPragmas[F]) {
      ...
      std::vector<Pragma*> SavedPragmas;
      std::copy_if(item.second.begin(), item.second.end(), std::back_inserter(SavedPragmas),
        [Type](const Pragma* Pragma) {
          return Pragma->getPragmaType() != Type;
        });
      ...
      // changes vectors of pragmas
      for (Pragma* Pragma : item.second) {
        if (std::find(SavedPragmas.begin(), SavedPragmas.end(), Pragma) == SavedPragmas.end()) {
          delete Pragma;
        }
      }
      item.second = NewPragmas;
